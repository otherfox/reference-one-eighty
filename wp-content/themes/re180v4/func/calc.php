...
function get_d_forms($atts,$content = null) {
	...
	if ($type=="cal"){
        $income = isset($_GET['income'])?$_GET['income']:0;
...
<tr>
<td class="calc_td_result" colspan="3"><input id="calc_result" class="calc_result" value="if (\''.$income.'\'=\'\') {this.value=\'\'} else {this.value=\''.$income.'\'}" type="text" name="income" onblur="if (this.value == \'\') {this.value = \'0\';}"
...
function get_calc() {
		...
		//net profit
		$np = isset($_GET['income']) ? $_GET['income'] : 0;
		$min_np = 113700;
		if ($np >= 1000000000) {
			$np = 0;
		}
		...
		//---------------- LLC/S-corp -----------------------------
		$per_sal = isset($_POST['percentValue'])?$_POST['percentValue']:0;
		if($per_sal > 0){
		}else{
			$per_sal = .6;
		}
		...
		$tax_sav = !empty($se_tax) ? round(($se_tax-$fica)/$se_tax,4) : 0;
